...
describe "Common::Text::CredentialFieldMergerBuilder" do
  ...
  let(:secrets_credentials) { UserConfig::Definitions::SecretsCredential.new("secrets", UserConfig::CredentialFactory.get_credential_query_lambda(secrets_user_config)) }
  ...
  it "should NOT build secrets credential when not exist" do 
    fields = builder.with_secrets(nil).build()
    fields.get_definitions_key().length().must_equal(0)
  end

  it "should build secrets credential" do
    given_secrets_credential("test_key", "test_value")
    fields = builder.with_secrets(secrets_credentials).build()
    definitions = fields.get_definitions_key()
    definitions.length().must_equal(1)
    definitions.must_include("[credential:secrets:test_key]")
  end

  it "should build merge fields for all secrets credentials" do
    given_secrets_credential("test_key", "test_value")
    given_secrets_credential("test_key2", "test_value2")
    fields = builder.with_secrets(secrets_credentials).build()
    definitions = fields.get_definitions_key()
    definitions.length().must_equal(2)
    definitions.must_include("[credential:secrets:test_key]")
    definitions.must_include("[credential:secrets:test_key2]")
  end
